...
for(exposure_i in exposure){
  ...
  for(outcome_i in outcome){
    # snp - outcome
    out <- read.table(paste0(pathResults,"InstrumentSelection/iv_",outcome_i,".txt")) %>%
  ...
  readr::write_delim(harmonised, paste0(pathResults,"MR_Results/harmonised_",exposure_i,".txt"))
  readr::write_delim(MR_result, paste0(pathResults,"MR_Results/MR_Results_",exposure_i,".txt"))
}
